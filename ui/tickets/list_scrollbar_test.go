...
func TestModel_CtrlDCtrlUPagesSidebar(t *testing.T) {
	...
	// Entry 0 is the "Open epics" section header, itself cursor-reachable —
	// initial selection lands there.
	...
	m = updated.(Model)
	if want := list.DefaultScroll; m.sidebarTree.SelectedIndex() != want {
	...
	// Clamps at the top: ctrl+u past the start stays at entry 0, the section
	// header.
	updated, _ = m.Update(tea.KeyPressMsg{Code: 'u', Mod: tea.ModCtrl})
	m = updated.(Model)
	if m.sidebarTree.SelectedIndex() != 0 {
