...
func (s *State) TabViewStateForViewContext(ctx nav.ViewContext) nav.ViewState {
	...
	case nav.TabStatus, nav.TabStash, nav.TabPRs, nav.TabTickets, nav.TabQueue:
		tabViewState.WorktreeRoot = ctx.WorktreeRoot
	...
	if tabID == nav.TabLog || tabID == nav.TabStatus || tabID == nav.TabStash {
...
func (s *State) Switch(vs nav.ViewState) nav.ViewState {
	tabVS := s.TabViewStateForViewContext(vs.Context())
	// ADR-0007: carry the active tab's worktree to the target when none was
	// specified, overriding per-tab memory so log and status always show the
	// same worktree after a tab switch.
	if vs.WorktreeRoot == "" {
		tabVS.WorktreeRoot = s.Active().WorktreeRoot
	}
	tabVS.FocusSubject = vs.FocusSubject
...
func (s *State) SetInitialTab(vs nav.ViewState) {
	tabVS := s.TabViewStateForViewContext(vs.Context())
	// Context() drops the filter/focus/scope options, so carry them over
	// explicitly (matches Open) — otherwise an InitialRoute filter like
	// `gx log -f` or `gx prs --all` is lost.
	tabVS.FocusSubject = vs.FocusSubject
	tabVS.FilterPath = vs.FilterPath
	tabVS.FilterStartLine = vs.FilterStartLine
	tabVS.FilterEndLine = vs.FilterEndLine
	tabVS.AllRepos = vs.AllRepos
	s.liveTab = tabVS.Tab
	s.activeTab = tabVS.Tab
	s.initMissingTabs()
	s.lastViewStateByTab[tabVS.Tab] = tabVS
...
func (s *State) initMissingTabs() {
	for _, kind := range []nav.TabID{nav.TabWorktrees, nav.TabLog, nav.TabStatus, nav.TabStash} {
		if _, ok := s.lastViewStateByTab[kind]; !ok {
			...
			// and needs none).
			if kind == nav.TabLog || kind == nav.TabStatus || kind == nav.TabStash {
...
func ResolveTabID(kind nav.TabID) nav.TabID {
	switch kind {
	case nav.TabWorktrees, nav.TabLog, nav.TabStatus, nav.TabStash, nav.TabPRs, nav.TabTickets, nav.TabQueue:
		return kind
	default:
