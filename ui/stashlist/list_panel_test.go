...
func runInit(m listPanel) listPanel {
	cmd := m.Init()
	if cmd == nil {
		return m
	}
	...
	return updated.(listPanel)
}

// send sends a message to the model and returns the updated listPanel.
func send(m listPanel, msg tea.Msg) listPanel {
	updated, _ := m.Update(msg)
	return updated.(listPanel)
}
